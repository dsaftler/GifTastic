...
function createButtons() {
  for (let i=0; i<topics.length; i++) {
    ...
    var newBtn = $('<button>' + topic + '</button>')
        .addClass('imgBtn')
        .attr("id",'topicbtn_' + i)
        .attr('onclick', 'getGif(this.textContent,false);')
...
function getGif(thisTopic){
 $("#getMore").hide();
 myTopic=thisTopic
  //alert("clicked "+myTopic );
  var queryURL = "http://api.giphy.com/v1/gifs/search?q=" +
    myTopic + "&offset=10&api_key=cQ6v8TyEzI3FgHn04lpj1mdBt9i4u5n8&limit=10";
  $.ajax({
    url: queryURL,
    method: "GET"
  })
...
function showGifs(imgSet,isAdd){
  ...
  // $('.gif').click(changeState());
    var getMoreBtn = $('<button>Get 10 More ?</button>')
       .attr("id", "getMore")
      .attr('onclick', 'getGif(myTopic,true);')
